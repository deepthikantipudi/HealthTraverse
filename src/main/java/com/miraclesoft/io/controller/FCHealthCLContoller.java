...
public class FCHealthCLContoller {
	...
	public ResponseEntity<?> recentClOfPatient(@PathVariable("patientId") long patientId,
			@Value("${clQuery}") String query) throws Exception {
		// System.out.println (query);

		List<Object[]> valueByPid = clRepository.findRecentValueByPid(patientId, query);
		long cl = Long.parseLong(String.valueOf(valueByPid.get(0)[0]));
		String category;
	...
	public List<AverageCholestrolEntity> getAverageCholestrolValues(@PathVariable("patientId") long patientId,
			@PathVariable("year") int year, @Value("${avgCL}") String query) throws Exception {
